...
public func ensureConfig(at fileURL: URL = configFile,
                          configGenerator: () -> Config = { generateFreshConfig() }) {
    guard !FileManager.default.fileExists(atPath: fileURL.path) else { return }
    try? FileManager.default.createDirectory(at: fileURL.deletingLastPathComponent(), withIntermediateDirectories: true)
    if let data = try? JSONEncoder().encode(configGenerator()) {
        try? data.write(to: fileURL)
    }
    log("Created default config at \(fileURL.path)")
}

public func loadConfig(from fileURL: URL = configFile,
                        configGenerator: () -> Config = { generateFreshConfig() }) -> Config {
    ensureConfig(at: fileURL, configGenerator: configGenerator)
